...
def insert_order(connection, orders):
    ...
    order_query = ("INSERT INTO orders (customer_name, total_price_of_order, date_time) VALUES (%s, %s, %s)")
    order_data = (orders['customer_name'], orders['total_price_of_order'], datetime.now())

    cursor.execute(order_query, order_data)
    order_id = cursor.lastrowid

    order_details_query = ("INSERT INTO order_details (orders_id, product_id, quantity, total_price) VALUES (%s, %s, %s, %s)")
...
def get_order_details(connection, orders_id):
    ...
    query = "SELECT * from order_details where orders_id = %s"
    ...
    query = "SELECT order_details.orders_id, order_details.quantity, order_details.total_price, "\
            ...
            "order_details.product_id = products.product_id where order_details.orders_id = %s"
